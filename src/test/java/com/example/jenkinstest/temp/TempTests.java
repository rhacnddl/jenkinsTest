package com.example.jenkinstest.temp;

import org.junit.jupiter.api.Test;

import java.util.ArrayList;
import java.util.List;

public class TempTests {

    @Test
    void testV1(){

        List<String> list = new ArrayList<>();
        System.out.println("hello new world");
        System.out.println("hello new world v2");
        System.out.println("hello new world v3");
        System.out.println("hello new world v4");
        System.out.println("hello new world v5");
        System.out.println("hello new world v6");
        System.out.println("hello new world v7");
        System.out.println("hello new world v8");
        System.out.println("hello new world v9");
        System.out.println("hello new world v10");
        System.out.println("hello new world v11");
        System.out.println("hello new world v12");

        //String str = list.get(0);

        //System.out.println(str);
    }
}
